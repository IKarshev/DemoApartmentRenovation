$(function(){
    ...
    $('body').on('click', '.add_new_section-btn', function(event){
        ...
        $(last_element).after(`
            <li class="draggable-list-item new_item">
                ...
                <a href="javascript:void(0);" class="detele-draggable-item new">
    ...
    $('body').on('click', '.detele-draggable-item', function(event){
        event.preventDefault();
        ...
        if( !$(this).hasClass('new') ){
            ...
            if( confirm('Точно удалить?') ){
                ...
                BX.ajax.runComponentAction('kontur:admin_settings', 'DeleteDraggableItem', {
                    mode: 'class',
                    data: {
                        "PROPERTY_ID":  PropertyId,
                    },
                }).then(function(response){
                    $(this_item).closest('.draggable-list-item').remove();
                });
            };
        }else{
