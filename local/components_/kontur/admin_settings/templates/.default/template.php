                            ...
                            <div class="draggable-list-container">
                                ...
                                <ul class="draggable-list" data-field_code="<?=$Field['CODE']?>">
                                    <?foreach ($Field['VALUES'] as $ItemKey => $ItemValue):
                                        $DefaultItems[] = $ItemValue['PROP_CODE'];
                                        ?>
                                        <li class="draggable-list-item" data-count="<?=$ItemValue['VALUE_CODE']?>">
                                            <div class="count"><?=$ItemKey+1?></div>
                                            <div class="name"><?=$ItemValue['NAME']?></div>
                                            <div class="code"><?=$ItemValue['VALUE_CODE']?></div>
                                            <input class="activity" type="checkbox" name="<?=$Field['CODE']."_ACTIVITY[]"?>" value="<?=$ItemValue['VALUE_CODE']?>" <?=($ItemValue['ACTIVITY']) ? 'checked' : ''?>>
                                            <a href="javascript:void(0);" class="detele-draggable-item" data-property_id="<?=$ItemValue['ID']?>">
